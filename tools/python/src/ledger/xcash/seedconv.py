...
from .dictionaries.languages import xcash_langs
...
def usage(c):
    print('''
Usage:
    python -m ledger.xcash.seedconv online|offline
...
def xcash_seed_to_xcash_keys(seed):
  l = 0x1000000000000000000000000000000014def9dea2f79cd65812631a5cf5d3ed
  printdbg('xcash_seed_to_xcash_keys: seed %s'%seed.hex())
  kh = keccak.new(digest_bits=256)
  kh.update(seed)
  b = kh.digest()  
  printdbg('xcash_seed_to_xcash_keys: b %s'%b.hex())
  ...
  printdbg('xcash_seed_to_xcash_keys: a %s'%a.hex())
...
def convert_mnemonic(language, ledger_mnemonic, passphrase):
    ...
    xcash_ki, xcash_ci = master_key_to_child_key(mkey, u"m/44'/128'/0'/0/0")
    xcash_seed = xcash_ki
    xcash_view_key, xcash_spend_key = xcash_seed_to_xcash_keys(xcash_seed)
    xcash_words = spendkey_to_words(xcash_spend_key, language)

    printdbg('seed: %s'%binascii.hexlify(s))
    printdbg('Km : %s'%binascii.hexlify(mkey[0]))
    printdbg('Cm:  %s'%binascii.hexlify(mkey[0]))
    printdbg('xcash_seed: %s'%binascii.hexlify(xcash_seed))
    printdbg('xcash view  keys: %s'%binascii.hexlify(xcash_view_key))
    printdbg('xcash spend keys: %s'%binascii.hexlify(xcash_spend_key))
    printdbg('xcash words: %d %s'%(len(xcash_words), ' '.join(xcash_words)))

    return xcash_words, xcash_view_key, xcash_spend_key,xcash_seed
...
def get_offline_seed(lang):
    ...
    electrum_words,  xcash_view_key, xcash_spend_key, xcash_seed = convert_mnemonic(lang, mnemonic, passphrase)
    print(u'''
    * Result:
      ---------------------------------------------------------------------------------------------------
    ...
    ''' .format (' '.join(electrum_words[0:8]),' '.join(electrum_words[8:16]),' '.join(electrum_words[16:]),
            binascii.hexlify(xcash_seed).decode(),
            binascii.hexlify(xcash_spend_key).decode(),
            binascii.hexlify(xcash_view_key).decode()))
...
def test():
    ...
    xcash_view_key, xcash_spend_key = xcash_seed_to_xcash_keys(binascii.unhexlify(u'DB9E57474BE8B64118B6ACF6ECEBD13F8F7C326B3BC1B19F4546573D6BAC9DCF'))
    print('xcash view  keys: %s'%binascii.hexlify(xcash_view_key))
    print('xcash spend keys: %s'%binascii.hexlify(xcash_spend_key))
    sys.exit(1);
