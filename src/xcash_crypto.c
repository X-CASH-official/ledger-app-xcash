...
void xcash_aes_derive(cx_aes_key_t *sk, unsigned char *R, unsigned char *a, unsigned char *b) {
    ...
    xcash_keccak_init_H();
    xcash_keccak_update_H(R, 32);
    xcash_keccak_update_H(a, 32);
    xcash_keccak_update_H(b, 32);
    xcash_keccak_update_H(R, 32);
    xcash_keccak_final_H(h1);

    xcash_keccak_H(h1,32,h1);
...
/* ---                                                                 --- */
/* ----------------------------------------------------------------------- */
/* thanks to knaccc and xcashmoo help on IRC #xcash-research-lab */
/* From Monero code
 *
 *  fe_sq2(v, u);            // 2 * u^2
 *  fe_1(w);
...
static void xcash_ge_fromfe_frombytes(unsigned char *ge , unsigned char *bytes) {
    ...
    #define Pxy   uv._Pxy
#else
    #define u   (G_xcash_vstate.io_buffer+0*32)
    #define v   (G_xcash_vstate.io_buffer+1*32)
    #define w   (G_xcash_vstate.io_buffer+2*32)
    #define x   (G_xcash_vstate.io_buffer+3*32)
    #define y   (G_xcash_vstate.io_buffer+4*32)
    #define z   (G_xcash_vstate.io_buffer+5*32)
    #define rX  (G_xcash_vstate.io_buffer+6*32)
    #define rY  (G_xcash_vstate.io_buffer+7*32)
    #define rZ  (G_xcash_vstate.io_buffer+8*32)

    //#define uv7 (G_xcash_vstate.io_buffer+9*32)
    //#define v3  (G_xcash_vstate.io_buffer+10*32)
    union {
        unsigned char _Pxy[65];
        struct {
            unsigned char _uv7[32];
...
#if XCASH_IO_BUFFER_LENGTH < (9*32)
#error  XCASH_IO_BUFFER_LENGTH is too small
#endif
#endif
...
void xcash_hash_to_scalar(unsigned char *scalar, unsigned char *raw) {
    xcash_keccak_F(raw,32,scalar);
    xcash_reduce(scalar, scalar);
}
...
void xcash_hash_to_ec(unsigned char *ec, unsigned char *ec_pub) {
    xcash_keccak_F(ec_pub, 32, ec);
    xcash_ge_fromfe_frombytes(ec, ec);
    xcash_ecmul_8(ec, ec);
}
...
void xcash_generate_keypair(unsigned char *ec_pub, unsigned char *ec_priv) {
    xcash_rng(ec_priv,32);
    xcash_reduce(ec_priv, ec_priv);
    xcash_ecmul_G(ec_pub, ec_priv);
}
...
void xcash_generate_key_derivation(unsigned char *drv_data, unsigned char *P, unsigned char *scalar) {
    xcash_ecmul_8k(drv_data,P,scalar);
}
...
void xcash_derivation_to_scalar(unsigned char *scalar, unsigned char *drv_data, unsigned int out_idx) {
    ...
    len_varint = xcash_encode_varint(varint+32, out_idx);
    len_varint += 32;
    xcash_keccak_F(varint,len_varint,varint);
    xcash_reduce(scalar, varint);
}
...
/* ---                                                                 --- */
/* ----------------------------------------------------------------------- */
void xcash_derive_secret_key(unsigned char *x,
                              unsigned char *drv_data, unsigned int out_idx, unsigned char *ec_priv) {
    unsigned char tmp[32];

    //derivation to scalar
    xcash_derivation_to_scalar(tmp,drv_data,out_idx);

    //generate
    xcash_addm(x, tmp, ec_priv);
    xcash_io_insert(tmp,32);
    xcash_io_insert(ec_priv,32);
...
/* ---                                                                 --- */
/* ----------------------------------------------------------------------- */
void xcash_derive_public_key(unsigned char *x,
    ...
    xcash_derivation_to_scalar(tmp,drv_data,out_idx);
    //generate
    xcash_ecmul_G(tmp,tmp);
    xcash_ecadd(x,tmp,ec_pub);
}
...
void xcash_generate_key_image(unsigned char *img, unsigned char *P, unsigned char* x) {
    unsigned char I[32];
    xcash_hash_to_ec(I,P);
    xcash_ecmul_k(img, I,x);
}
...
/* --- ok                                                              --- */
/* ----------------------------------------------------------------------- */
void xcash_derive_subaddress_public_key(unsigned char *x,
                                         unsigned char *pub, unsigned char* drv_data, unsigned int index) {
  unsigned char scalarG[32];

  xcash_derivation_to_scalar(scalarG , drv_data, index);
  xcash_ecmul_G(scalarG, scalarG);
  xcash_ecsub(x, pub, scalarG);
}
...
void xcash_get_subaddress_spend_public_key(unsigned char *x,unsigned char *index) {
    ...
    xcash_get_subaddress_secret_key(x, G_xcash_vstate.a, index);
    // M = m*G
    xcash_secret_key_to_public_key(x,x);
    // D = B + M
    xcash_ecadd(x,x,G_xcash_vstate.B);
 }
/* ----------------------------------------------------------------------- */
/* ---                                                                 --- */
/* ----------------------------------------------------------------------- */
void xcash_get_subaddress(unsigned char *C, unsigned char *D, unsigned char *index) {
    ...
    // C = a*D
    xcash_ecmul_k(C,D,G_xcash_vstate.a);
}
...
void xcash_get_subaddress_secret_key(unsigned char *sub_s, unsigned char *s, unsigned char *index) {
    ...
    xcash_keccak_F(in, sizeof(in), sub_s);
    xcash_reduce(sub_s, sub_s);
}
...
void xcash_addm(unsigned char *r, unsigned char *a, unsigned char *b) {
    ...
    xcash_reverse32(ra,a);
    xcash_reverse32(rb,b);
...
int xcash_bamount2str(unsigned char *binary,  char *str, unsigned int str_len) {    
    return xcash_amount2str(xcash_bamount2uint64(binary), str,str_len);
}
...
int xcash_vamount2str(unsigned char *binary,  char *str, unsigned int str_len) {
   return xcash_amount2str(xcash_vamount2uint64(binary), str,str_len);
}
