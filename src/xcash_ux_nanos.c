...
void ui_info(const char* msg1, const char* msg2, const void *menu_display, unsigned int value) {
  os_memset(&G_xcash_vstate.ui_dogsays[0], 0, sizeof(ux_menu_entry_t));
  G_xcash_vstate.ui_dogsays[0].callback = menu_display;
  G_xcash_vstate.ui_dogsays[0].userid   = value;
  G_xcash_vstate.ui_dogsays[0].line1    = msg1;
  G_xcash_vstate.ui_dogsays[0].line2    = msg2;

  os_memset(&G_xcash_vstate.ui_dogsays[1],0, sizeof(ux_menu_entry_t));
  UX_MENU_DISPLAY(0, G_xcash_vstate.ui_dogsays, NULL);
};
*/
/* ----------------------------- FEE VALIDATION ----------------------------- */
...
const bagl_element_t* ui_menu_fee_validation_preprocessor(const ux_menu_entry_t* entry, bagl_element_t* element) {
  ...
  if (entry == &ui_menu_fee_validation[0]) {
    if(element->component.userid==0x22) {      
      element->text = G_xcash_vstate.ux_amount;
    }
  }
  return element;
}
...
void ui_menu_fee_validation_action(unsigned int value) {
  ...
  xcash_io_insert_u16(sw);  
  xcash_io_do(IO_RETURN_AFTER_TX);
...
#define WORDS N_xcash_pstate->words
...
const bagl_element_t* ui_menu_words_preprocessor(const ux_menu_entry_t* entry, bagl_element_t* element) {
  if ((entry->userid >= 0) && (entry->userid <25)) {
    ...
    if(element->component.userid==0x21) {      
      element->text = N_xcash_pstate->words[entry->userid];
    }
  
    if ((element->component.userid==0x22)&&(entry->userid<24)) {      
      element->text = N_xcash_pstate->words[entry->userid+1];
    }
  }
...
const bagl_element_t* ui_menu_validation_preprocessor(const ux_menu_entry_t* entry, bagl_element_t* element) {
  ...
  if (entry == &ui_menu_validation[1]) {
    if(element->component.userid==0x22) {      
      element->text = G_xcash_vstate.ux_fees;
    }
  }
#endif
  ...
  if (entry == &ui_menu_validation[1]) {
    if(element->component.userid==0x22) {     
      os_memset(G_xcash_vstate.ux_menu, 0, sizeof(G_xcash_vstate.ux_menu)) ;
      os_memmove(G_xcash_vstate.ux_menu, G_xcash_vstate.ux_address+11*0, 11);
      element->text = G_xcash_vstate.ux_menu;
    }
  }
  if (entry == &ui_menu_validation[2]) {
    os_memset(G_xcash_vstate.ux_menu, 0, sizeof(G_xcash_vstate.ux_menu)) ;
    if(element->component.userid==0x21) {     
      os_memmove(G_xcash_vstate.ux_menu, G_xcash_vstate.ux_address+11*1, 11);      
    }
    if(element->component.userid==0x22) {     
      os_memmove(G_xcash_vstate.ux_menu, G_xcash_vstate.ux_address+11*2, 11);
    }
    element->text = G_xcash_vstate.ux_menu;
  }
  if (entry == &ui_menu_validation[3]) {
    os_memset(G_xcash_vstate.ux_menu, 0, sizeof(G_xcash_vstate.ux_menu)) ;
    if(element->component.userid==0x21) {     
      os_memmove(G_xcash_vstate.ux_menu, G_xcash_vstate.ux_address+11*3, 11);      
    }
    if(element->component.userid==0x22) {     
      os_memmove(G_xcash_vstate.ux_menu, G_xcash_vstate.ux_address+11*4, 11);
    }
    element->text = G_xcash_vstate.ux_menu;
  }
  if (entry == &ui_menu_validation[4]) {
    os_memset(G_xcash_vstate.ux_menu, 0, sizeof(G_xcash_vstate.ux_menu)) ;
    if(element->component.userid==0x21) {     
      os_memmove(G_xcash_vstate.ux_menu, G_xcash_vstate.ux_address+11*5, 11);      
    }
    if(element->component.userid==0x22) {     
      os_memmove(G_xcash_vstate.ux_menu, G_xcash_vstate.ux_address+11*6, 11);
    }
    element->text = G_xcash_vstate.ux_menu;
  }
  if (entry == &ui_menu_validation[5]) {
    os_memset(G_xcash_vstate.ux_menu, 0, sizeof(G_xcash_vstate.ux_menu)) ;
    if(element->component.userid==0x21) {     
      os_memmove(G_xcash_vstate.ux_menu, G_xcash_vstate.ux_address+11*7, 11);      
    }
    if(element->component.userid==0x22) {     
      os_memmove(G_xcash_vstate.ux_menu, G_xcash_vstate.ux_address+11*8, 7);
    }
    element->text = G_xcash_vstate.ux_menu;
  }
...
unsigned int ui_export_viewkey_prepro(const  bagl_element_t* element) {
  if (element->component.userid == 1) {
    snprintf(G_xcash_vstate.ux_menu, sizeof(G_xcash_vstate.ux_menu), "Export");
    return 1;
  }
  if (element->component.userid == 2) {
    snprintf(G_xcash_vstate.ux_menu, sizeof(G_xcash_vstate.ux_menu), "View Key");
    return 1;
  }
  snprintf(G_xcash_vstate.ux_menu, sizeof(G_xcash_vstate.ux_menu), "Please Cancel");
  return 1;
}
...
const bagl_element_t* ui_menu_network_preprocessor(const ux_menu_entry_t* entry, bagl_element_t* element) {
  os_memset(G_xcash_vstate.ux_menu, 0, sizeof(G_xcash_vstate.ux_menu));  
  if ((entry == &ui_menu_network[2]) && (element->component.userid==0x20) && (N_xcash_pstate->network_id == TESTNET)) {
    os_memmove(G_xcash_vstate.ux_menu, "Test Network  ", 14);
    G_xcash_vstate.ux_menu[13] = '+';
    element->text = G_xcash_vstate.ux_menu;
  }
  if ((entry == &ui_menu_network[3]) && (element->component.userid==0x20) && (N_xcash_pstate->network_id == STAGENET)) {
    os_memmove(G_xcash_vstate.ux_menu, "Stage Network ", 14);
    G_xcash_vstate.ux_menu[13] = '+';
    element->text = G_xcash_vstate.ux_menu;
  }
  if ((entry == &ui_menu_network[4]) && (element->component.userid==0x20) && (N_xcash_pstate->network_id == MAINNET)) {
    os_memmove(G_xcash_vstate.ux_menu, "Main Network  ", 14);
    G_xcash_vstate.ux_menu[13] = '+';
    element->text = G_xcash_vstate.ux_menu;
  }
  return element;
...
void ui_menu_reset_action(unsigned int value) {
  ...
  xcash_nvm_write(N_xcash_pstate->magic, magic, 4);
  xcash_init();
  ui_menu_main_display(0);
}
/* ------------------------------- SETTINGS UX ------------------------------- */
...
const ux_menu_entry_t ui_menu_info[] = {
  ...
  {NULL,  NULL,                 -1, NULL,          "App  " XSTR(XCASH_VERSION),  NULL, 0, 0},
...
const bagl_element_t* ui_menu_main_preprocessor(const ux_menu_entry_t* entry, bagl_element_t* element) {
  if (entry == &ui_menu_main[0]) {
    if(element->component.userid==0x20) {  
      os_memset(G_xcash_vstate.ux_menu, 0, sizeof(G_xcash_vstate.ux_menu));
      os_memmove(G_xcash_vstate.ux_menu, "< Monero: ", 10);
      xcash_base58_public_key(G_xcash_vstate.ux_menu+10, G_xcash_vstate.A,G_xcash_vstate.B, 0);
      G_xcash_vstate.ux_menu[10+95+0] = ' ';
      G_xcash_vstate.ux_menu[10+95+1] = '>';
